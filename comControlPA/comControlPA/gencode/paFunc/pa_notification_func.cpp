...
void* funcPA_HMIShowupStatus(int argc, char *argv[])
{
   for(int i=1;i<argc;i++)
   {
      printf("%s\n",argv[i]);
   ...
   if(argc<(2+1))
   {
      printf("too few parameters,run app like this: ./winPAApi PA_HMIShowupStatus value");
   }else
   {
      unsigned char value;
      value = atoi(argv[2]);
      PA_HMIShowupStatus.setValue(value);
   }
}
...
void initpa_notificationFuncMap()
{
   ...
   mFuncPtr.insert(make_pair<string, long>("PA_HMIShowupStatus", (long)&funcPA_HMIShowupStatus));
